...
class ProductMyUpdateActivity : BaseActivity(), ProductMyUpdateContract.View {

  companion object {
    private const val DETAIL = "mydetail"
    fun newInstance(context: Context, detail: ProductDetailUiModel): Intent {
      val intent = Intent(context, ProductMyUpdateActivity::class.java)
  ...
  @Inject
  lateinit var presenter: ProductMyUpdateContract.Presenter
  ...
  private fun userAction() {
    ...
    binding.btUpload.setOnClickListener {
      selectImage(this@ProductMyUpdateActivity)
    }
    ...
    binding.btSave.setOnClickListener {
      ...
      image?.let { img ->
        presenter.updateProductByOwner(detail?.id.toString(), req, img)
      }
    }
  }

  override fun setData(details: ProductDetailUiModel) {
    Glide.with(this).load(URL_PRODUCT + details.productImage).fitCenter().into(binding.ivProduct)
    with(binding) {
  ...
  private fun submit(){
    ...
    image?.let { detail?.id?.let { it1 -> presenter.updateProductByOwner(it1, req, it) } }
  }
}